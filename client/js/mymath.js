...
function getA(m) {
    //строим матрицу А дополняя кажду строку единицой
    for (i in m) {
        m[i].push(1);
    }
    return m;
...
function _determinant(A)   // определитель
{
...
function _matrixCofactor(i,j,A)   //Алгебраическое дополнение матрицы
{
...
function _adjugateMatrix(A)      //Присоединенная матрица
{
...
function inverse(A)   // Обратная матрица
{
...
//проверка принадлежности точки симплексу
//-1 не принадлежит; 0 - принадлежит границе симплекса; 1 - лежит внутри симплекса
...
//находим осевые диаметры симплекса
...
//находим концы осевых диаметров
function endPoints(l, a) {
    var sum = _findSum(l), i, j, k, s = [], t = [], result = [], n = l.length;
    for (i=0; i < n-1; i++) {
        s[i] = []; t[i] = [];
        for (j=0; j < n; j++) {
        ...
        s[i] = s[i].map(function(num) {return num/sum[i]});
        t[i] = t[i].map(function(num) {return num/sum[i]});
    }
    result = [[],[]];
    for (k=0; k < n-1; k++) {
        result[0][k] = [];
...
//получаем вершины единичного куба в текущем пространстве
function _getQ(n, f) {
...
//находим минимальный коэффициент гомотетии симплекса
function findKsi(l) {
    ...
    for (i=0; i<lambda.length; i++) {
        lambda[i] = lambda[i].map(function(num){return -num;});
        arr[i] = Math.max.apply(Math, lambda[i]);
    }
    return l.length * Math.max.apply(Math, arr) + 1;
}
//находим норму интерполяционного проектора
function findP(l) {
    var q = _getQ(l.length), i, k, j, arr = [], lambda = [];
    for (i=0; i< l.length; i++) {
        lambda[i] = [];
...
//находим коэффициент гомотетии симплекса
...
function vectorEndPoints(a, l, v) {
    var m = _findM(l, v), sum = _findMSum(m), i, j, k, alpha = [], beta = [], result = [[],[]], n = l.length;
    for (i=0; i<n; i++) {
...
function findVectorD(l ,v) {
    var m = _findM(l, v), sum = _findMSum(m), norm = _findVectorNorm(v);
    return (2 * norm) / sum;
}

function findVectorKsi(l ,v, a) {
    var m = _findM(l, v), sum = _findMSum(m), norm = _findVectorNorm(v), det = _determinant(a),
...
function findX(l, a) {
    ...
    result = result.map(function(num){return num/(sum - 2)});
    return result;
}
